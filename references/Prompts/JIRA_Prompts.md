...
Find all issues (Epics, Stories, Sub-tasks) in `[PROJECT-KEY]` that mention 'authentication'.
Show me the entire hierarchy (Epic -> Story -> Sub-task) for `[USER-STORY-KEY]`.
List all issues that are blocked by `[ISSUE-KEY]`.
What are the dependencies for `[ISSUE-KEY]`?"

**Key Considerations for Developers:**

*   **Issue Types:** Be mindful of the exact issue types configured in your Jira instance (e.g., "Story" vs. "User Story", "Sub-task" vs. "Subtask").
*   **Linking:** Understand how your team links Epics to Stories (often via an "Epic Link" field) and Stories to Sub-tasks (via "Parent" field).
*   **JQL Power:** For more complex queries involving multiple levels of hierarchy, JQL is indispensable. For example, to find all sub-tasks of stories within a specific epic: `issueFunction in subtasksOf("issueFunction in issuesInEpics('EPIC-KEY')")`.
*   **MCP Tools:** The `jira_get_issue` tool can retrieve an issue's details, including its Epic link and parent information, which is crucial for understanding its place in the hierarchy. `jira_create_issue` allows you to specify parent issues for subtasks or link to epics using `additional_fields`.

These prompts should help developers efficiently manage and track their work within the Epic-User Story-Sub Task structure.
